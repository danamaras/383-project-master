This is the template for CMPT 383 projects using a Virtual Machine, providing some basics to get started with various languages in a consistent environment.
...
If you need a service running (for example, a server process), the *easiest* thing to do is an `execute` block in your chef recipe that simply runs the process. There is a commented-out example in the provided recipe with the comment "simply starting an executable".

The better solution is to create a [systemd service](https://www.freedesktop.org/software/systemd/man/systemd.service.html) that will take control of starting/stopping your server as the system reboots. In order to do that, you need a service description (example provided as `cookbooks/polyglot/files/hello.service`). Then you can install this service with your chef recipe: see the example with the comment "creating a systemd service".

Once you have a "systemd service" created, you can control it with systemd commands like:
